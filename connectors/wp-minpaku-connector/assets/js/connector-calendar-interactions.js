...
(function(window, document) {
    ...
    class ConnectorCalendarInteractions {
        constructor(rootElement, options = {}) {
            ...
            const localizedTexts = (typeof window.wpmcCalendarData !== 'undefined' && window.wpmcCalendarData.texts)
                ? window.wpmcCalendarData.texts
                : {};
        ...
        debug(message, data = null) {
            if (typeof window.MCS_DEBUG !== 'undefined' && window.MCS_DEBUG) {
                if (data) {
                    console.debug(`[WPMC Calendar] ${message}`, data);
                } else {
                    console.debug(`[WPMC Calendar] ${message}`);
                }
            }
        }
         ...
         */
        handleClick(e) {
            const dayElement = e.target.closest('.wpmc-day');
        ...
        startLongPressSelection(cell) {
            ...
            // Add visual feedback
            this.root.classList.add('wpmc-selecting-range', 'is-selecting');
        }
        ...
        updateVisualSelection() {
            ...
            this.root.querySelectorAll('.wpmc-selected, .wpmc-selected-start, .wpmc-selected-end, .wpmc-selected-range')
                .forEach(cell => {
                    cell.classList.remove('wpmc-selected', 'wpmc-selected-start', 'wpmc-selected-end', 'wpmc-selected-range', 'is-selected');
            ...
            if (checkinCell) {
                checkinCell.classList.add('wpmc-selected-start', 'is-selected');
            ...
            if (this.state.selectedCheckout) {
                ...
                if (checkoutCell) {
                    checkoutCell.classList.add('wpmc-selected-end', 'is-selected');
                ...
                this.root.querySelectorAll('.wpmc-day[data-ymd]').forEach(cell => {
                    const cellDate = new Date(cell.dataset.ymd);
                    if (cellDate > checkinDate && cellDate < checkoutDate) {
                        cell.classList.add('wpmc-selected-range', 'is-selected');
                    }
                });
            }
        }
         ...
         */
        initNavigation() {
            const navButtons = this.root.querySelectorAll('.wpmc-nav-button');
        ...
        updateNavigationState() {
            const monthsGrid = this.root.querySelector('.wpmc-calendar-months-grid');
            const prevBtn = this.root.querySelector('.wpmc-nav-prev');
            const nextBtn = this.root.querySelector('.wpmc-nav-next');
        ...
        getQuotePanel() {
            ...
            let panel = this.root.querySelector('.wpmc-quote-panel');
            if (panel) return panel;

            const calendarId = this.root.getAttribute('data-calendar-id');
            if (calendarId) {
                panel = document.querySelector(`.wpmc-quote-panel[data-calendar-id="${calendarId}"]`);
                if (panel) return panel;
            }

            if (this.options.mode === 'modal') {
                const modal = this.root.closest('.modal, .dialog, [role="dialog"]');
                panel = modal?.querySelector('.wpmc-quote-panel');
                if (panel) return panel;
            } else {
                panel = this.root.parentNode.querySelector('.wpmc-quote-panel');
                if (panel) return panel;
            }
        ...
        createQuotePanelElement() {
            ...
            panel.className = 'wpmc-quote-panel';
            ...
            panel.innerHTML = `
                <div class="wpmc-quote-header">
                    ...
                    <button class="wpmc-clear-selection-btn" aria-label="${this.texts.clearSelection || '選択をクリア'}">×</button>
                </div>
            ...
            const clearBtn = panel.querySelector('.wpmc-clear-selection-btn');
            if (clearBtn) {
                clearBtn.addEventListener('click', () => this.clearSelection());
            }
        ...
        async fetchQuote() {
            ...
            const payload = {
                ...
                start_date: this.state.selectedCheckin,
                end_date: this.state.selectedCheckout,
            ...
            try {
                const quoteUrl = (typeof window.wpmcCalendarData !== 'undefined' && window.wpmcCalendarData.restUrl)
                    ? window.wpmcCalendarData.restUrl + 'quote'
                    : '/wp-json/minpaku-connector/v1/quote';
                ...
                if (typeof window.MCS_DEBUG !== 'undefined' && window.MCS_DEBUG) {
                    if (typeof window.__WPMC_DEBUG === 'undefined') {
                        window.__WPMC_DEBUG = {};
                    }
                    window.__WPMC_DEBUG.quote = function(data) {
                        console.log('[WPMC Debug] Quote data:', data || { payload, response: responseData });
                    };
                }
            ...
            const html = `
                <div class="wpmc-quote-summary">
                    <div class="wpmc-quote-dates">
                        ...
                        <span class="wpmc-nights-count">${nights}${this.texts.nightsLabel}</span>
                    </div>
                </div>
                <div class="wpmc-quote-breakdown">
                    <table class="wpmc-quote-table">
                        <tbody>
                            <tr>
                                <td>${this.texts.accommodationFee || '宿泊料金'}</td>
                                <td>¥${formatter.format(quoteData.base_total || 0)}</td>
                            </tr>
                            ${quoteData.cleaning_fee > 0 ? `
                            <tr>
                                <td>${this.texts.cleaningFee || '清掃料金'}</td>
                                <td>¥${formatter.format(quoteData.cleaning_fee)}</td>
                            </tr>
                            ` : ''}
                            <tr class="wpmc-total-row">
                                <td><strong>${this.texts.totalLabel || '合計金額'}</strong></td>
                                <td><strong>¥${formatter.format(quoteData.grand_total || 0)}</strong></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
        ...
        displayQuoteError(errorMessage) {
            ...
            const html = `
                <div class="wpmc-quote-error">
                    <h4>${this.texts.errorTitle || 'エラー'}</h4>
                    <p>${errorMessage}</p>
                </div>
            `;
        ...
        clearSelection() {
            ...
            this.updateVisualSelection();
            this.root.classList.remove('wpmc-selecting-range', 'is-selecting');
    ...
    window.WPMCCalendarInteractions = {
        init: function(options = {}) {
            const selector = options.selector || '.connector-calendar[data-interactions="modern"]';
    ...
    if (typeof window.MCS_DEBUG !== 'undefined' && window.MCS_DEBUG) {
        window.__WPMC_DEBUG = window.__WPMC_DEBUG || {};
        window.__WPMC_DEBUG.status = function() {
            const calendars = document.querySelectorAll('.connector-calendar[data-interactions="modern"]');
